...
module NATS
  ...
  class Client
    ...
    @subscriptions : Hash(Int64, Subscription) = {} of Int64 => Subscription
    ...
    # }
    # ```
    def publish(subject : String, data : Payload = Bytes.empty, reply_to : String? = nil, headers : Headers? = nil) : Nil
      ...
      write do
        ...
        if headers
          ...
          header_length = nats_header_preamble.bytesize + 2 # 2 extra bytes for final CR+LF
          each_as_array headers do |key, values|
            values.each do |value|
              header_length += key.bytesize + value.bytesize + 4 # 2 extra bytes for ": " and 2 for CR+LF
            end
          end
          @io << ' ' << header_length
    ...
    # iterates over a hash but converts string values to an array of strings
    private def each_as_array(h : Headers)
      h.each do |k, v|
        case v
        when String
          yield k, [v]
